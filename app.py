...
class PersonalGifCollection(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=user_collection, backref=db.backref('personalGifCollections', lazy='dynamic'), lazy='dynamic') # This model should also have a many to many relationship with the Gif model (one gif might be in many personal collections, one personal collection could have many gifs in it).
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=search_gifs, backref=db.backref('search_term', lazy='dynamic'), lazy='dynamic') # This model should have a many to many relationship with gifs (a search will generate many gifs to save, and one gif could potentially appear in many searches)
...
def get_gifs_from_giphy(search_string):
    ...
    pdict = {"api_key":api_key, "q":search_string, "limit":5} # limit of 5 gifs
    r = requests.get(baseurl, pdict)
    list_of_gifs = json.loads(r.text)["data"]
    return(list_of_gifs)
...
def get_or_create_search_term(term):
    ...
    if searchterm:
        #print('term is already saved')
        return searchterm

    else:
        #print('new term, adding to db')
        searchterm = SearchTerm(term=term)

        gifsearch = get_gifs_from_giphy(term)
        for gif in gifsearch:
            t = gif['title']
            url = gif['embed_url']
            new_gif = get_or_create_gif(t, url)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    collection_of_gifs = PersonalGifCollection.query.filter_by(name=name, user_id=current_user.id).first()

    if collection_of_gifs:
        return collection_of_gifs
    else:
        collection_of_gifs = PersonalGifCollection.query.filter_by(name=name, user_id=current_user.id, gifs=gif_list)

        for gif in gif_list:
            collection_of_gifs.gifs.append(gif)

        db.session.add(collection_of_gifs)
        db.session.commit()
...
def index():
    ...
    if f.validate_on_submit():
        search_term = f.search.data
        t = get_or_create_search_term(search_term)
...
def create_collection():
    ...
    if request.method == "POST":
        ...
        gifobj = [get_gif_by_id(int(id))]
        get_or_create_collection(name=form.name.data, current_user=current_user, gif_list=gifobj)
        redirect(url_for('collections', id_num=current_user))
